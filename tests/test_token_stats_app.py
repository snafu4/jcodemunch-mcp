...
def test_build_stats_payload_price_env_overrides(monkeypatch, tmp_path: Path) -> None:
    ...
    assert payload["pricing_usd_per_token"]["claude_opus"] == 20 / 1_000_000
    assert payload["pricing_usd_per_token"]["gpt5_latest"] == 30 / 1_000_000
...
def test_build_stats_payload_zero_price_env_is_allowed(monkeypatch, tmp_path: Path) -> None:
    savings_file = tmp_path / "_savings.json"
    savings_file.write_text(json.dumps({"total_tokens_saved": 500}), encoding="utf-8")
    monkeypatch.setenv("JCODEMUNCH_OPUS_PRICE", "0")

    payload = build_stats_payload(str(tmp_path))

    assert payload["pricing_usd_per_token"]["claude_opus"] == 0
    assert payload["total_cost_avoided"]["claude_opus"] == 0
